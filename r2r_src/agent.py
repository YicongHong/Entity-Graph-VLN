...
from utils import padding_idx, add_idx, Tokenizer, print_progress, get_glove_matrix
import utils
import model
import param
...
class Seq2SeqAgent(BaseAgent):
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        ctx, h_t, c_t, h_graph_action = self.encoder(seq, seq_lengths)
        ctx_mask = seq_mask
        ...
        for t in range(self.episode_len):
            ...
            # get candidate object features
            cand_object_feat = self.get_cand_object_feat(perm_obs)
            ...
            h_t, c_t, logit, h1, action_tilde = self.decoder(input_a_t, f_t, candidate_feat, cand_object_feat,
                                               h_t, h1, c_t, h_graph_action,
                                               ctx, ctx_mask, perm_obs, train_rl,
            ...
            a_t_graph = a_t.clone(); a_t_graph = a_t_graph.detach()
            for i, atg in enumerate(a_t_graph):
                if atg == args.ignoreid:
                    a_t_graph[i] = 0
            h_graph_action = action_tilde[range(batch_size), a_t_graph, :]
        ...
        if train_rl:
            ...
            if speaker is not None:
                candidate_feat[..., :-args.angle_feat_size] *= noise
                f_t[..., :-args.angle_feat_size] *= noise
            last_h_, _, _, _, _ = self.decoder(input_a_t, f_t, candidate_feat, cand_object_feat,
                                            h_t, h1, c_t, h_graph_action,
                                            ctx, ctx_mask, perm_obs, train_rl,
                                            speaker is not None)
            rl_loss = 0.
