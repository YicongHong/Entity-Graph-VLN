...
class Speaker():
    ...
    def train(self, iters):
        for i in range(iters):
            ...
            utils.print_progress(i, iters+1, prefix='Progress:', suffix='Complete', bar_length=50)
    ...
    def from_shortest_path(self, viewpoints=None, get_first_feat=False):
        ...
        while not ended.all():
            ...
            img_feats.append(self.listener._feature_variable(obs)) # a list of [batch, 36, 2176] features, one for a step
    ...
    def teacher_forcing(self, train=True, features=None, insts=None, for_listener=False):
        ...
        if features is not None:
            ...
            obs = self.env._get_obs()
            batch_size = len(obs)
            (img_feats, can_feats), lengths = self.from_shortest_path()      # Image Feature (panoramic, teacher_selected)

            ''' Post LSTM layer: encode visual sequence for inferring new languages ''' # (batch_size, total_steps, 512)
            ctx = self.encoder(can_feats, img_feats, lengths)
